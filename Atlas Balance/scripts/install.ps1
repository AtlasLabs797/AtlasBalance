...
if (-not (Test-Path $apiExe) -or -not (Test-Path $watchdogExe)) {
    throw "Esta carpeta no es el paquete instalable. Genera o descarga AtlasBalance-V-01.07-win-x64.zip. Ejecuta el instalador desde la carpeta descomprimida que contiene api\AtlasBalance.API.exe, watchdog\AtlasBalance.Watchdog.exe, scripts e install.cmd."
}
