...
export default function DashboardPage() {
  ...
  const principalQuery = useQuery({
    queryKey: queryKeys.dashboard.principal({ usuarioId, paisId: selectedPaisId || null, divisaPrincipal }),
    queryFn: ({ signal }) =>
      api.get<DashboardPrincipal>('/dashboard/principal', {
        params: { divisaPrincipal, paisId: selectedPaisId || undefined },
        signal,
      }).then((res) => res.data),
    enabled: Boolean(allowed && usuarioId),
    staleTime: QUERY_STALE_TIMES.DASHBOARD_MS,
  });

  const evolucionQuery = useQuery({
    queryKey: queryKeys.dashboard.evolucion({ usuarioId, paisId: selectedPaisId || null, divisaPrincipal, periodo }),
    queryFn: ({ signal }) =>
  ...
  const saldosDivisaQuery = useQuery({
    queryKey: queryKeys.dashboard.saldosDivisa({ usuarioId, paisId: selectedPaisId || null, divisaPrincipal }),
    queryFn: ({ signal }) =>
  ...
  const loading = principalQuery.isLoading || evolucionQuery.isLoading || saldosDivisaQuery.isLoading;
  const error =
    principalQuery.error ? extractErrorMessage(principalQuery.error, 'No se pudo cargar el dashboard.') :
    evolucionQuery.error ? extractErrorMessage(evolucionQuery.error, 'No se pudo cargar el dashboard.') :
    saldosDivisaQuery.error ? extractErrorMessage(saldosDivisaQuery.error, 'No se pudo cargar el dashboard.') :
    null;
