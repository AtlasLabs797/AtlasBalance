...
{
    private const int DefaultMaxRowsPerExport = 50_000;
    private const int AbsoluteMaxRowsPerExport = 200_000;
    ...
    public async Task<Exportacion> ExportarCuentaAsync(Guid cuentaId, TipoProceso tipo, Guid? iniciadoPorId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var maxRows = await GetIntConfigValueAsync("export_max_rows", DefaultMaxRowsPerExport, cancellationToken);
            maxRows = Math.Clamp(maxRows, 1, AbsoluteMaxRowsPerExport);
            var extractoCount = await _dbContext.Extractos.CountAsync(e => e.CuentaId == cuentaId, cancellationToken);
            if (extractoCount > maxRows)
            {
                ...
                throw new ExportacionTooLargeException(
                    $"La exportacion contiene {extractoCount} filas y supera el limite configurado de {maxRows}. Filtra o divide la exportacion.");
            }
            ...
            foreach (var extracto in extractos)
            {
                ...
                worksheet.Cell(row, 6).Value = extracto.Checked ? "Si" : "No";
                worksheet.Cell(row, 7).Value = extracto.Flagged ? "Si" : "No";
            ...
            _dbContext.NotificacionesAdmin.Add(new NotificacionAdmin
            {
                Id = Guid.NewGuid(),
                Tipo = "EXPORTACION",
                Mensaje = $"Exportacion completada: {Path.GetFileName(filePath)}",
                Leida = false,
        ...
        catch (ExportacionTooLargeException)
        {
    ...
    private static string ResolveSafeDirectory(string rawPath, string configKey)
    {
        ...
        {
            throw new InvalidOperationException($"Configuracion '{configKey}' vacia.");
        }

        var trimmed = rawPath.Trim();
        if (trimmed.Contains("..", StringComparison.Ordinal))
        {
            throw new InvalidOperationException($"Configuracion '{configKey}' contiene segmentos de traversal.");
        }

        // V-02.07: rechazar rutas de red (UNC). Path.IsPathRooted("\\\\host\\share") devuelve
        // true, asi que sin este filtro un valor UNC ya guardado en BD escribiria las
        // exportaciones fuera de la maquina. Se revalida aqui aunque el controlador ya filtre.
        ...
        {
            throw new InvalidOperationException($"Configuracion '{configKey}' no es una ruta valida.", ex);
        }
...
public sealed class ExportacionTooLargeException : Exception
{
    public ExportacionTooLargeException(string message) : base(message) { }
}
