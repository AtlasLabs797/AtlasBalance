...
public sealed class ExportacionesController : ControllerBase
{
    ...
    public async Task<IActionResult> Manual([FromBody] ExportacionManualRequest request, CancellationToken cancellationToken)
    {
        ...
        catch (ExportacionTooLargeException ex)
        {
            _logger.LogWarning(ex, "Exportacion manual bloqueada por tamano para cuenta {CuentaId}", request.CuentaId);
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(StatusCodes.Status500InternalServerError, new { error = "No se pudo generar la exportacion. Revisa la configuracion del servidor o avisa al administrador." });
        }
    }
