...
public sealed class IntegrationOpenClawController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var extractoIds = extractosScope.Select(x => x.Id);
        ...
        var pageExtractoIds = rows
            .Where(x => x.EntidadTipo == "EXTRACTOS" && x.EntidadId.HasValue)
            .Select(x => x.EntidadId!.Value)
            .Distinct()
            .ToList();
        var extractoMap = pageExtractoIds.Count == 0
            ? new Dictionary<Guid, Guid>()
            : await extractosScope
                .Where(x => pageExtractoIds.Contains(x.Id))
                .ToDictionaryAsync(x => x.Id, x => x.CuentaId, cancellationToken);
