...
public sealed class TitularesControllerTests
{
    ...
    {
        var controller = new TitularesController(db, new UserAccessService(db, new CacheService(new MemoryCache(new MemoryCacheOptions()), NullLogger<CacheService>.Instance), Options.Create(new CachingOptions())), TestAuditService.Create(db));
