...
public class AlertaServiceTests
{
    ...
    [Fact]
    public async Task EvaluateSaldoPostAsync_Should_Not_Send_Duplicate_Inside_Configured_Cooldown()
    {
        await using var db = BuildDbContext();
        ...
        var previousAlertDate = DateTime.UtcNow.AddHours(-2);
        ...
        db.Configuraciones.Add(new Configuracion
        {
            ...
            Valor = "24",
            Tipo = "number",
            Descripcion = "Cooldown de saldo bajo"
        });
        db.Usuarios.Add(new Usuario
        {
            Id = actorId,
        ...
        db.AlertasSaldo.Add(new AlertaSaldo
        {
            ...
            FechaUltimaAlerta = previousAlertDate
        });
        db.AlertaDestinatarios.Add(new AlertaDestinatario
        {
            Id = Guid.NewGuid(),
        ...
        emailService.Messages.Should().BeEmpty();
        auditService.Entries.Should().BeEmpty();
        (await db.AlertasSaldo.SingleAsync(x => x.Id == alertaId))
            .FechaUltimaAlerta.Should().Be(previousAlertDate);
    }
    ...
    public async Task EvaluateSaldoPostAsync_Should_Not_Update_LastAlert_When_Email_Fails()
    {
        ...
        db.Titulares.Add(new Titular { Id = titularId, Nombre = "Titular Uno", Tipo = TipoTitular.EMPRESA });
        ...
        db.AlertaDestinatarios.Add(new AlertaDestinatario { Id = Guid.NewGuid(), AlertaId = alertaId, UsuarioId = actorId });
        db.Extractos.Add(new Extracto
        {
            Id = Guid.NewGuid(),
            CuentaId = cuentaId,
        ...
        var emailService = new RecordingEmailService { FailSaldoBajo = true };
        var auditService = new RecordingAuditService();
        var sut = new AlertaService(db, emailService, auditService, NullLogger<AlertaService>.Instance);
    ...
    private sealed class RecordingEmailService : IEmailService
    {
        ...
        public bool FailSaldoBajo { get; init; }
