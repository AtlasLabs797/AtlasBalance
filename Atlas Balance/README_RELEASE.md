# Atlas Balance V-02-02 - release Windows x64
...
El ZIP `main` de GitHub no sirve como instalador. Usa `AtlasBalance-V-02-02-win-x64.zip`; dentro deben existir `api\AtlasBalance.API.exe` y `watchdog\AtlasBalance.Watchdog.exe`.
...
```powershell
C:\AtlasBalance\update.cmd -PackagePath C:\Temp\AtlasBalance-V-02-02-win-x64 -InstallPath C:\AtlasBalance
```
